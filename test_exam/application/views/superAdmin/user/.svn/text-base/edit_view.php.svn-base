...
$full_name = array("name" => "full_name", "id" => "full_name", "size" => "30", "value" => $info["name"]);
$username = array('name' => 'username', 'id' => 'fname', 'size' => '30', 'value' => $info["username"], "readonly" => "readonly");
...
$address = array('name' => 'address', 'id' => 'address', 'size' => '30', 'value' => $info["address"], );
$phone = array('name' => 'phone', 'id' => 'phone', 'size' => '30', 'value' => $info["phone"], );
$email = array('name' => 'email', 'id' => 'email', 'size' => '30', 'value' => $info["email"], );
...
$stared_day = date('Y-m-d', strtotime($orga['started_date']));
$expired_day = date('Y-m-d', strtotime($orga['expired_date']));
?>
...
<div id="box_entry">
	...
	<form name="frmEdit" id="frmEdit" action="" method="post" enctype="multipart-formdata">
		<fieldset>
			...
			<select name="month">
				<?php for ($i=1;$i<=12;$i++){ ?> <option value="<?php echo $i?>"<?php if ($i == $month)	echo "selected";?> ><?php echo $i?>	</option><?php } ?>
			</select>
			<select name="year">
				<?php for ($i=1980;$i<=2010;$i++){?><option value="<?php echo $i?>" <?php if ($i == $year) echo "selected";?> ><?php echo $i?></option>	<?php } ?>
			</select>
			<br />
