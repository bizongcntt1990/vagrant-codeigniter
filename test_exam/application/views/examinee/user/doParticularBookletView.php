<!-- Author: Ngo Anh Tuan
This file is for doing a particular test
...
//Programmed　at 2:40 10/4 by Ngo Anh Tuan
...
function update_clock(countdown_div, new_value,functionWhenTimeout) {
	...
	{
		disabledByExpired=' disabled = "disabled"';
...
function displayQuestion(questionNumber)
{
	...
	document.getElementById("question").innerHTML="<strong>Question " + (questionNumber+1)+":"+ BookletJSArray[questionNumber][1]+" </strong>	";
		...
		else
		{
			...
			buttonToDisplay="<button onclick='prev("+questionNumber+")'><< 前</button><button onclick='submit()' >>>>あなたの答えを提出<<<<</button>";
			else
...
function displayQuestionFixed(questionNumber)
{
	...
	document.getElementById("question").innerHTML="<strong>Question " + (questionNumber+1)+":"+ BookletJSArray[questionNumber][1]+" </strong>	";
		...
		buttonToDisplay="";
			...
			if(questionNumber+1==<?php echo $numberOfQuestion?>)
			{
				buttonToDisplay="<button onclick='forceSubmit()' >>>>あなたの答えを提出<<<<</button>";
...
function forceSubmit()
{
	...
	endDoingEachQuestion=date.getTime();
	time=(endDoingEachQuestion-startDoingEachQuestion)/1000;
	if(confirm("いま答えを提出しますか？提出の後, 答えが修正できない. 続行しますか？"+time))
	{
		//date=new Date();
		//endDoingEachQuestion=date.getTime();
		//get the last question
...
{
	//alert("テストの残りの時間があまりない！あなたの答えをすぐ提出して下さい！");
...
<div id="box_display">
                  ...
                  <p align="center">採点するタイム: <?php echo $testTime?> minutes.</p>  
